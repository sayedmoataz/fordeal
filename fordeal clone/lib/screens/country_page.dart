...
import 'package:secondtask/main.dart';
...
class _CountryPageState extends State<CountryPage> {
  ...
  Future<void>insertCountryLang(
    ...
    await http.get(uri).then((value)async{
      print(value.body);
      setState((){
        insertCountryLangModel=InsertCountryLangModel.fromJson( jsonDecode(value.body));
      });
  ...
  Future<void>getCountries()async{countries.clear();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: countriesModel==null?Center(child: CircularProgressIndicator(),):Center(
        child: Column(
            ...
            Container(
              ...
              child:TextButton(
                  onPressed: ()async {
                    ...
                    insertCountryLang(
                        countriesModel!.data.where((element) => element.countryName==dropdownCountry).toList()[0].id.toString(),languages.values.toList()[languages.keys.toList().indexOf(dropdownLang)].toString(), latitude, longitude,await GetMac.macAddress ).then((value) => {
                          setState((){
                            isLoading=false;
                          }),
                      CacheHelper.putData(key: 'countryId', value: insertCountryLangModel!.countryId.toString()).then((value) {
                        if(value){
